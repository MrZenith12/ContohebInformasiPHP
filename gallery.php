        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading">
                    <h2>Galeri Terbaru <?php echo date("Y"); ?></h2>
                </div>
            </div>
            
            <?php
            // Hitung jumlah total berita dari tabel tbl_galeri
            $sql_total_galeri = "SELECT COUNT(*) AS total FROM tbl_galeri";
            $result_total_galeri = $koneksi->query($sql_total_galeri);
            $total_berita = $result_total_galeri->fetch_assoc()['total'];
            $galeri_per_halaman = 6; // Jumlah berita yang ingin ditampilkan per halaman
            $total_halaman = ceil($total_berita / $galeri_per_halaman);
            $halaman_saat_ini = isset($_GET['halaman']) ? $_GET['halaman'] : 1;

            // Hitung offset berdasarkan nomor halaman dan jumlah berita per halaman
            $offset = ($halaman_saat_ini - 1) * $galeri_per_halaman;

            // Query untuk mengambil berita terbaru dari tabel tbl_galeri dengan offset dan limit sesuai halaman yang diminta
            $sql = "SELECT * FROM tbl_galeri ORDER BY tanggal DESC LIMIT $galeri_per_halaman OFFSET $offset";
                            ...
                            <div class="col-lg-12">
                                <div class="meeting-item">
                                    ...
                                    <div class="down-content">
                                        <div class="date">
                                            <h6><?php echo date('F j, Y', strtotime($row['tanggal'])); ?> <span></span></h6>
                                        </div>
                                        <h4><?php echo $judul_galeri; ?></h4>
                                        <p><?php echo $deskripsiShort; ?> <a href="detail/detailgaleri.php?id_galeri=<?php echo $row['id_galeri']; ?>">Selengkapnya</a></p>
                                    </div>
                                </div>
                            </div>
                        </div>
